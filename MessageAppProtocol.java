...
public class ATMProtocol {
     ...
     * Payload:
     * <ul>
     * <li>(int) starting_balance
     * <li>(int) account_number
     * </ul>
     *
     * <p>
     * The {@code account_number} is an optional field. If the user supplies a
     * desired account number, the client should send it in the {@code
     * account_number} field. The {@code account_number} field should be between
     * 1 and 100 if valid. If the user does not supply a desired account number,
     * the value should be {@code -1}. After the server processes the creation
     ...
     * <li>(int) account_number
     * </ul>
     * The {@code account_number} is the number of the account the server just
     * created.
     */
     ...
     * <ul>
     * <li>invalid desired account number,
     * <li>unavailable desired account number,
     * <li>invalid starting balance.
     * </ul>
     *
     */
     ...
     * <ul>
     * <li>invalid account number,
     * <li>nonzero account balance.
     * </ul>
     */
    public static final byte DELETE_FAILURE = 0x22;

    /**
     * Account deposit request from a client to a server. This message should
     * only be sent to a server; if received by a client it should be ignored. <br>
     * Opcode: {@code 0x30} <br>
     * Payload:
     * <ul>
     ...
     *
     * <p>
     * The {@code account_number} is the number of the account that should
     * receive the deposit. The {@code amount} should be a positive whole-number
     * amount of USD to be deposited in the specified account. After the server
     * processes the deposit request, the server sends one of the following
     * acknowledgments to the client:
     * <ul>
     * <li> {@link #DEPOSIT_SUCCESS}, if account deposit succeeds.
     * <li>{@link #DEPOSIT_FAILURE}, if account deposit fails.
     * </ul>
     */
    public static final byte DEPOSIT_REQUEST = 0x30;

    /**
     * Deposit success. Server response to a client's {@link #DEPOSIT_REQUEST}. <br>
     * Opcode: {@code 0x31} <br>
     * Payload:
     * <ul>
     * <li>(int) balance
     ...
     */
    public static final byte DEPOSIT_SUCCESS = 0x31;

    /**
     * Deposit failure. Server response to a client's {@link #DEPOSIT_REQUEST}. <br>
     * Opcode: {@code 0x32} <br>
     * Payload:
     * <ul>
     * <li>(String) reason
     * </ul>
     * This message indicates that the server rejects the
     * {@link #DEPOSIT_REQUEST} for some reason, for example:
     * <ul>
     * <li>invalid account number,
     * <li>invalid deposit amount.
     * </ul>
     *
     * Note that we assume a client has less than 2^32-1 dollars in a single
     * account. This would likely not be the case even if a client had 2^32-1
     * dollars due to FDIC limitations as well as practical money-management
     * issues (one would likely play the market with a portion, etc.).
     */
    public static final byte DEPOSIT_FAILURE = 0x32;
    ...
    /**
     * Balance request from a client to a server. Requests the remaining balance
     * in a given account. This message should only be sent to a server; if
     ...
     *
     * <p>
     * The {@code account_number} is the number of the account whose balance
     * should be returned. After the server processes the account balance check
     * request, the server sends one of the following acknowledgments to the
     * client:
     * <ul>
     * <li> {@link #BALANCE_SUCCESS}, if account balance check succeeds.
     * <li>{@link #BALANCE_FAILURE}, if account balance check fails.
     * </ul>
     */
    public static final byte BALANCE_REQUEST = 0x50;

    /**
     * Balance check success. Server response to a client's
     * {@link #BALANCE_REQUEST}. <br>
     * Opcode: {@code 0x51} <br>
     * Payload:
     * <ul>
     * <li>(int) balance
     * </ul>
     *
     * The {@code balance} returned reflects the current balance in the
     * specified account.
     */
    public static final byte BALANCE_SUCCESS = 0x51;

    /**
     * Deposit failure. Server response to a client's {@link #BALANCE_REQUEST}. <br>
     * Opcode: {@code 0x52} <br>
     * Payload:
     * <ul>
     * <li>(String) reason
     * </ul>
     * This message indicates that the server rejects the
     * {@link #BALANCE_REQUEST} for some reason, for example:
     * <ul>
     * <li>invalid account number.
     * </ul>
     */
    public static final byte BALANCE_FAILURE = 0x52;

    /**
     * Request from a client to a server to disconnect and shut down the ATM.
     ...
     * {@link #END_SESSION_SUCCESS} to the client.
     */
    public static final byte END_SESSION = 0x60;

    /**
     * End session success. Server response to a client's {@link #END_SESSION}. <br>
     * Opcode: {@code 0x61} <br>
     * Payload: None.
     */
     ...
     * {@link #END_SESSION_SUCCESS} message. Opcode: {@code Ox62}<br>
     * Payload: None.
     */
